...
for (const b of psrc.split(/\{\s*\n\s*id:/).slice(1)) {
  ...
  all.push({ slug: s2, name: g(/name: "([^"]+)"/) || "", desc: g(/description: "([^"]+)"/) || "", price: +(g(/price: (\d+)/) || 0), unit: g(/unit: "([^"]+)"/) || "", pid: g(/barkiryProductId: "(p\d+)"/) || null, ingredients: g(/ingredients: "([^"]+)"/) || null, usage: g(/usage: "([^"]+)"/) || null });
}
...
// 발키리 실거래 범위 (2026-07-17 신설): fetch-barkiri-prices.mjs 수집분. 있으면 글 필수 반영 대상.
let barkiri = null;
try {
  const pdb = JSON.parse(fs.readFileSync("data/barkiri-prices.json", "utf8"));
  if (mine.pid && pdb[mine.pid] && !pdb[mine.pid].error) barkiri = { pid: mine.pid, ...pdb[mine.pid] };
} catch {}
if (barkiri) { harvest(barkiri.lowPrice + "원"); harvest(barkiri.highPrice + "원"); harvest(String(barkiri.storeCount)); harvest(barkiri.fetchedAt); harvest(String(barkiri.highPrice - barkiri.lowPrice) + "원"); }

const brief = {
  slug, cat, status: me.status, issues: me.issues,
  barkiriRange: barkiri, // {lowPrice, highPrice, storeCount} — 가격 섹션에 "실거래 N원~M원, 인증약국 K곳" 필수 서술
  ...
  bodyRules: {
    ...
    가격섹션: barkiri ? "발키리 실거래 " + barkiri.lowPrice + "원~" + barkiri.highPrice + "원(인증약국 " + barkiri.storeCount + "곳) 반드시 본문 서술 + 가격 포지션(동일성분 내 순위) + 버튼" : "가격 포지션(동일성분 내 순위·범위) + 발키리/네이버 버튼. 실거래가 미확인 시 '기준가' 명시",
