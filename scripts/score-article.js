...
for (const k of HIGHER_BETTER) {
  let floor = bench[k].floor;
  // 2026-07-03: 숫자밀도 기준은 소스 숫자밀도에 연동 (소스에 없는 숫자 창작 금지 원칙과 정합)
  if (k === "numPer100") {
    try {
      const src = JSON.parse(require("fs").readFileSync(`source-data/${slug}.json`, "utf8"));
      const t = ["efcyQesitm","useMethodQesitm","atpnQesitm","seQesitm"].map((f)=>src[f]||"").join(" ");
      const srcDen = t.length > 200 ? (t.match(/\d/g)||[]).length / (t.length/100) : 99;
      const dyn = Math.max(1.0, Math.min(floor, srcDen * 0.9));
      if (dyn < floor) { console.log(`  (숫자밀도 기준 소스연동: ${floor} → ${dyn.toFixed(2)} — 소스밀도 ${srcDen.toFixed(2)})`); floor = +dyn.toFixed(2); }
    } catch {}
  }
  if (m[k] < floor) fails.push(`${KO[k]}: ${m[k]} < 기준 ${floor} (표준 중앙값 ${bench[k].median})`);
}
for (const k of LOWER_BETTER) {
