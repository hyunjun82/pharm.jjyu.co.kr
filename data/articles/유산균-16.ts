import type { SpokeArticle } from "@/lib/types";

export const spokes16: SpokeArticle[] = [
  // 1. V21하루good유산균
  {
    slug: "V21하루good유산균",
    categorySlug: "유산균",
    title: "V21 하루 good 유산균 성분 효과 | 자주빛 분홍색 분말 복용법 주의사항 총정리",
    h1: "V21 하루 good 유산균 성분 효과 | 자주빛 분홍색 분말 복용법 주의사항 총정리",
    metaDescription: "V21 하루 good 유산균의 성분, 효과, 복용법, 주의사항을 정리했습니다. 대웅제약이 만든 10억 CFU 자주빛 분홍색 분말로 보관 시 변색이 생길 수 있는 제품입니다.",
    description: "",
    heroDescription: "대웅제약이 만든 자주빛 분홍색 유산균이 궁금하다면 이 제품을 볼 수 있어요. V21 하루 good 유산균은 10억 CFU 프로바이오틱스를 자주빛 분홍색 분말 2g 포에 담은 제품이에요. 보관 중 자연적인 변색이 생길 수 있지만 품질에는 이상이 없다는 안내가 인상적인 제품이에요.",
    datePublished: "2026-04-04",
    dateModified: "2026-04-04",
    sections: [
      {
        title: "V21 하루 good 유산균 성분은 무엇인가요?",
        content: "고유의 향미가 있고 이미·이취가 없는 자주빛 분홍색 분말이에요. 1포(2,000mg)에 프로바이오틱스 10억 CFU가 들어 있어요. 흰색·노랑색이 대부분인 분말 유산균 제품과 달리 자주빛 분홍색이라는 독특한 색상이 이 제품의 가장 눈에 띄는 특징이에요.",
        ingredients: [
          { type: "주성분", name: "프로바이오틱스", amount: "10억 CFU/2,000mg당", role: "유산균 증식·유해균 억제, 배변활동 원활, 장 건강" },
        ],
      },
      {
        title: "V21 하루 good 유산균 효과가 나타나려면 얼마나 걸리나요?",
        content: "프로바이오틱스는 유산균 증식 및 유해균 억제, 배변활동 원활, 장 건강에 도움을 줄 수 있어요. 추가 성분 없이 장 건강에 집중한 단순 구성이에요."
      },
      {
        title: "V21 하루 good 유산균 복용법은 어떻게 되나요?",
        content: "1일 1회, 1회 1포(2g)를 직접 또는 물과 함께 섭취하면 돼요. 물 없이 그대로 먹어도 되고 물에 타서 마셔도 돼요. 자주빛 분홍색 분말이라 음료에 타면 색이 물들 수 있어요."
      },
      {
        title: "V21 하루 good 유산균 주의사항은 무엇인가요?",
        content: "질환이 있거나 의약품을 복용 중이라면 전문가와 상담하세요. 알레르기 체질은 개인에 따라 과민반응이 나타날 수 있어요. 어린이가 임의로 섭취하지 않도록 지도하고, 이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요."
      },
      {
        title: "V21 하루 good 유산균 보관할 때 주의할 점은 무엇인가요?",
        content: "직사광선 및 고온다습한 곳을 피해 서늘한 곳에 보관하세요. 보관 상태에 따라 자연적인 변색이 발생할 수 있지만, 제품 품질에는 이상이 없으니 안심하고 섭취하면 돼요. 영·유아 손에 닿지 않는 곳에 보관하세요. 유통기한은 제조일로부터 18개월이에요."
      }
    ],
    products: [],
    faq: [
      {
        question: "분말 색이 변했는데 먹어도 괜찮나요?",
        answer: "제품 안내에 보관 상태에 따라 자연적인 변색이 발생할 수 있으나 품질에는 이상이 없다고 명시되어 있어요. 다만 변색이 심하거나 이미·이취가 생겼다면 섭취를 피하고 구매처에 문의하는 게 좋아요."
      },
      {
        question: "자주빛 분홍색 색소가 인공 색소인가요?",
        answer: "소스 데이터에는 색소 관련 별도 정보가 없어요. 자연 색택인지 색소 첨가인지는 제품 상세 성분표를 확인하거나 제조사에 문의하는 게 정확해요."
      },
      {
        question: "이상 반응이 생기면 어떻게 해야 하나요?",
        answer: "이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 알레르기 반응이나 소화 이상이 나타나면 성분에 대한 과민반응을 의심해볼 수 있어요."
      }
    ]
  },

  // 2. W밸런스유산균
  {
    slug: "W밸런스유산균",
    categorySlug: "유산균",
    title: "W밸런스 유산균 성분 효과 | 셀렌 100μg·아연·비타민D 복합 캡슐 복용법 총정리",
    h1: "W밸런스 유산균 성분 효과 | 셀렌 100μg·아연·비타민D 복합 캡슐 복용법 총정리",
    metaDescription: "W밸런스 유산균의 성분, 효과, 복용법, 주의사항을 정리했습니다. 50억 CFU에 셀렌 100μg·아연 8.5mg·비타민D 10μg이 담긴 400mg 투명 경질캡슐 제품입니다.",
    description: "",
    heroDescription: "유산균에 비타민D·아연·셀렌까지 한 캡슐에 담은 제품을 찾는다면 이 제품이 맞을 수 있어요. 50억 CFU에 비타민D 10μg, 아연 8.5mg, 셀렌 100μg을 400mg 투명 경질캡슐에 담았어요. 특히 셀렌 100μg은 다른 제품보다 높은 함량이에요. 반드시 충분한 물과 함께 섭취해야 하는 캡슐 제품이에요.",
    datePublished: "2026-04-04",
    dateModified: "2026-04-04",
    sections: [
      {
        title: "W밸런스 유산균 성분은 무엇인가요?",
        content: "흰 분홍색 내용물을 함유한 투명한 경질캡슐이에요. 1캡슐(400mg)에 프로바이오틱스 50억 CFU, 비타민D 10μg, 아연 8.5mg, 셀렌 100μg이 들어 있어요. 셀렌 100μg은 다른 제품의 16~55μg보다 훨씬 높은 함량이에요. 장 건강, 뼈 건강(비타민D), 면역(아연), 항산화(셀렌) 4가지 기능을 400mg 한 캡슐에 담았어요.",
        ingredients: [
          { type: "주성분", name: "프로바이오틱스", amount: "50억 CFU/400mg당", role: "유산균 증식·유해균 억제, 배변활동 원활, 장 건강" },
          { type: "주성분", name: "비타민D", amount: "10μg/400mg당", role: "칼슘·인 흡수, 뼈 형성과 유지, 골다공증 위험 감소" },
          { type: "주성분", name: "아연", amount: "8.5mg/400mg당", role: "정상적인 면역기능, 정상적인 세포분열" },
          { type: "주성분", name: "셀렌", amount: "100μg/400mg당", role: "기능성 원료" },
        ],
      },
      {
        title: "W밸런스 유산균 효과가 나타나려면 얼마나 걸리나요?",
        content: "프로바이오틱스는 유산균 증식 및 유해균 억제, 배변활동 원활, 장 건강에 도움을 줄 수 있어요. 비타민D는 칼슘과 인이 흡수되고 이용되는 데 필요하고, 뼈의 형성과 유지에 필요하며, 골다공증 발생 위험 감소에 도움을 줄 수 있어요. 아연은 정상적인 면역기능과 세포분열에 필요해요. 셀렌은 유해산소로부터 세포를 보호하는 데 필요한 성분이에요."
      },
      {
        title: "W밸런스 유산균 복용법은 어떻게 되나요?",
        content: "1일 1회, 1회 1캡슐을 충분한 물과 함께 섭취하면 돼요. 캡슐이 목에 걸리거나 불편할 수 있으니 반드시 물과 함께 드세요. 섭취 시 위장장애나 소화불량 증상이 있으면 즉시 섭취를 중단하세요."
      },
      {
        title: "W밸런스 유산균 주의사항은 무엇인가요?",
        content: "비타민D가 포함되어 있어 고칼슘혈증이 있거나 의약품을 복용 중이라면 전문가와 상담하세요. 섭취 시 목에 걸리거나 불편할 수 있으니 반드시 물과 함께 섭취하세요. 위장장애나 소화불량이 생기면 즉시 중단하세요. 제품에 이상이 있으면 섭취하지 마세요. 알레르기 체질은 원료 성분을 확인한 후 섭취하세요. 이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요."
      },
      {
        title: "W밸런스 유산균 보관할 때 주의할 점은 무엇인가요?",
        content: "수분 및 열에 의해 영향을 받을 수 있으니 직사광선을 피해 서늘한 곳에 보관하세요. 어린이 손에 닿지 않는 곳에 보관하세요. 유통기한은 제조일로부터 18개월이에요."
      }
    ],
    products: [],
    faq: [
      {
        question: "셀렌 100μg이면 과하지 않나요?",
        answer: "셀레늄 1일 상한 섭취량은 성인 기준 400μg이에요. 이 제품의 100μg은 상한 이하지만, 다른 영양제에서도 셀레늄을 섭취하고 있다면 총 섭취량이 초과되지 않는지 확인하는 게 좋아요."
      },
      {
        question: "캡슐이 목에 걸릴 수 있다고 하는데 어떻게 하면 되나요?",
        answer: "충분한 물(150mL 이상)을 마시면서 캡슐을 삼키면 걸리는 현상을 줄일 수 있어요. 앉거나 선 자세에서 물을 충분히 마신 뒤 캡슐을 넣고 바로 물을 추가로 드세요."
      },
      {
        question: "이상 반응이 생기면 어떻게 해야 하나요?",
        answer: "이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 위장장애나 소화불량이 생겨도 즉시 중단하세요."
      }
    ]
  },

  // 3. W엔젤유산균
  {
    slug: "W엔젤유산균",
    categorySlug: "유산균",
    title: "W엔젤 유산균 성분 효과 | 아연 2.55mg 흰분홍색 분말 복용법 주의사항 총정리",
    h1: "W엔젤 유산균 성분 효과 | 아연 2.55mg 흰분홍색 분말 복용법 주의사항 총정리",
    metaDescription: "W엔젤 유산균의 성분, 효과, 복용법, 주의사항을 정리했습니다. 1억 CFU에 아연 2.55mg이 담긴 흰분홍색 분말 제품입니다.",
    description: "",
    heroDescription: "면역 지원을 위한 아연과 장 건강을 한 포에 담고 싶다면 이 제품이 맞을 수 있어요. W엔젤 유산균은 흰분홍색 분말 2g 포에 프로바이오틱스 1억 CFU와 아연 2.55mg을 담은 제품이에요. 물 없이도 섭취할 수 있어 편리한 분말 제형이에요.",
    datePublished: "2026-04-04",
    dateModified: "2026-04-04",
    sections: [
      {
        title: "W엔젤 유산균 성분은 무엇인가요?",
        content: "고유의 향미가 있고 이미·이취가 없는 흰분홍색 분말이에요. 1포(2g)에 프로바이오틱스 1억 CFU와 아연 2.55mg이 들어 있어요. 함량은 저함량이지만 아연으로 면역 기능까지 지원하는 복합 구성이에요.",
        ingredients: [
          { type: "주성분", name: "프로바이오틱스", amount: "소스 데이터 확인 필요", role: "유산균 증식·유해균 억제, 배변활동 원활, 장 건강" },
        ]
      },
      {
        title: "W엔젤 유산균 효과가 나타나려면 얼마나 걸리나요?",
        content: "프로바이오틱스는 유산균 증식 및 유해균 억제, 배변활동 원활, 장 건강에 도움을 줄 수 있어요. 아연은 정상적인 면역기능과 세포분열에 필요한 성분이에요. 장 건강과 면역을 함께 지원하는 구성이에요."
      },
      {
        title: "W엔젤 유산균 복용법은 어떻게 되나요?",
        content: "1일 1회, 1회 1포를 그대로 또는 충분한 물과 함께 섭취하면 돼요. 분말이라 물 없이도 먹을 수 있지만, 충분한 물과 함께 드시면 더 수월해요."
      },
      {
        title: "W엔젤 유산균 주의사항은 무엇인가요?",
        content: "질환이 있거나 의약품을 복용 중이라면 전문가와 상담하세요. 알레르기 체질은 개인에 따라 과민반응이 나타날 수 있어요. 어린이가 임의로 섭취하지 않도록 지도하고, 이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요."
      },
      {
        title: "W엔젤 유산균 보관할 때 주의할 점은 무엇인가요?",
        content: "직사광선 및 고온다습한 곳을 피해 건조하고 서늘한 곳에 실온 보관하세요. 유통기한은 제조일로부터 18개월이에요."
      }
    ],
    products: [],
    faq: [
      {
        question: "1억 CFU도 장 건강에 충분한가요?",
        answer: "1억 CFU도 프로바이오틱스 기능성 인정 기준 내에 있어요. 처음 유산균을 시작하거나 낮은 함량부터 시도하고 싶은 분께 맞는 수준이에요."
      },
      {
        question: "분말을 물 없이 먹어도 괜찮나요?",
        answer: "그대로 먹어도 되도록 설계된 제품이에요. 다만 충분한 물과 함께 드시면 분말이 더 잘 내려가고 유산균이 장까지 원활하게 도달하는 데 도움이 돼요."
      },
      {
        question: "이상 반응이 생기면 어떻게 해야 하나요?",
        answer: "이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 소화 이상이나 알레르기 반응이 나타나면 성분에 대한 과민반응일 수 있어요."
      }
    ]
  },

  // 4. jw중외제약신SYN퍼스트바이오틱스프리바이오틱스프로바이오틱스100억유산균
  {
    slug: "jw중외제약신SYN퍼스트바이오틱스프리바이오틱스프로바이오틱스100억유산균",
    categorySlug: "유산균",
    title: "jw중외제약 SYN 100억 유산균 성분 효과 | 프락토올리고당·아연 복용법 주의사항 총정리",
    h1: "jw중외제약 SYN 100억 유산균 성분 효과 | 프락토올리고당·아연 복용법 주의사항 총정리",
    metaDescription: "jw중외제약 SYN 퍼스트바이오틱스 100억 유산균의 성분, 효과, 복용법, 주의사항을 정리했습니다. 100억 CFU·프락토올리고당 3,000mg·아연 8.5mg이 담긴 5,000mg 분말 포 제품입니다.",
    description: "",
    heroDescription: "100억 유산균에 프리바이오틱스와 아연까지 한 포에 담은 제품을 찾는다면 이 제품이 맞을 수 있어요. jw중외제약이 선보인 퍼스트바이오틱스 제품으로, 100억 CFU 프로바이오틱스에 프락토올리고당 3,000mg과 아연 8.5mg을 5,000mg 포에 담았어요. 고함량에 프리바이오틱스까지 더한 복합 구성이에요.",
    datePublished: "2026-04-04",
    dateModified: "2026-04-04",
    sections: [
      {
        title: "jw중외제약 SYN 100억 유산균 성분은 무엇인가요?",
        content: "고유의 향미가 있고 이미·이취가 없는 노랑 하양 분말이에요. 1포(5,000mg)에 프로바이오틱스 100억 CFU, 프락토올리고당 3,000mg, 아연 8.5mg이 들어 있어요. 100억 고함량에 프리바이오틱스(먹이)와 아연(면역)까지 더한 3중 복합 구성이에요. 5,000mg 대용량 포는 프락토올리고당이 많이 들어 있기 때문이에요.",
        ingredients: [
          { type: "주성분", name: "프로바이오틱스", amount: "100억 CFU/5,000mg당", role: "유산균 증식·유해균 억제, 배변활동 원활, 장 건강" },
          { type: "주성분", name: "프락토올리고당", amount: "3,000mg/5,000mg당", role: "프리바이오틱스로 유익균 성장 지원" },
          { type: "주성분", name: "아연", amount: "8.5mg/5,000mg당", role: "정상적인 면역기능, 정상적인 세포분열" },
        ],
      },
      {
        title: "jw중외제약 SYN 100억 유산균 효과가 나타나려면 얼마나 걸리나요?",
        content: "프락토올리고당은 장내 유익균 증식 및 배변활동 원활에 도움을 줄 수 있어요. 프로바이오틱스는 유산균 증식 및 유해균 억제, 배변활동 원활, 장 건강에 도움을 줄 수 있어요. 아연은 정상적인 면역기능과 세포분열에 필요한 성분이에요. 고함량 유산균과 프리바이오틱스의 시너지 구성에 면역 지원까지 포함된 제품이에요."
      },
      {
        title: "jw중외제약 SYN 100억 유산균 복용법은 어떻게 되나요?",
        content: "1일 1회, 1회 1포를 섭취하면 돼요. 5,000mg 포로 용량이 크기 때문에 물에 타서 마시는 게 더 편해요. 미지근하거나 차가운 물을 사용하세요."
      },
      {
        title: "jw중외제약 SYN 100억 유산균 주의사항은 무엇인가요?",
        content: "질환이 있거나 의약품을 복용 중이라면 전문가와 상담하세요. 알레르기 체질은 과민반응이 나타날 수 있어요. 어린이가 임의로 섭취하지 않도록 지도하세요. 이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 프락토올리고당으로 인해 가스참, 트림, 복통, 복부팽만감이 발생할 수 있어요."
      },
      {
        title: "jw중외제약 SYN 100억 유산균 보관할 때 주의할 점은 무엇인가요?",
        content: "직사광선 및 고온다습한 곳을 피해 서늘한 실온에 보관하세요. 어린이 손에 닿지 않는 곳에 두세요. 유통기한은 제조일로부터 18개월이에요."
      }
    ],
    products: [],
    faq: [
      {
        question: "5,000mg 포가 너무 크지 않나요?",
        answer: "프락토올리고당 3,000mg이 포함되어 포 용량이 큰 편이에요. 물에 타서 마시면 편하게 섭취할 수 있어요. 1포를 반으로 나눠 먹는 방식은 권장하지 않아요."
      },
      {
        question: "배가 더부룩하면 어떻게 해야 하나요?",
        answer: "프락토올리고당이 처음 섭취 시 가스나 복부팽만감을 유발할 수 있어요. 가벼운 불편함은 적응 기간 이후 줄어드는 경우가 많아요. 심하거나 지속되면 즉시 섭취를 중단하고 전문가와 상담하세요."
      },
      {
        question: "이상사례가 생기면 어떻게 해야 하나요?",
        answer: "이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 소화 이상이나 알레르기 반응이 나타나면 섭취를 중단하는 게 먼저예요."
      }
    ]
  },

  // 5. jw중외제약혈당바나바유산균프로바이오틱스Pro
  {
    slug: "jw중외제약혈당바나바유산균프로바이오틱스Pro",
    categorySlug: "유산균",
    title: "jw중외제약 혈당 바나바 유산균 Pro 성분 효과 | 코로솔산 식후 혈당 복용법 총정리",
    h1: "jw중외제약 혈당 바나바 유산균 Pro 성분 효과 | 코로솔산 식후 혈당 복용법 총정리",
    metaDescription: "jw중외제약 혈당 바나바 유산균 프로바이오틱스 Pro의 성분, 효과, 복용법, 주의사항을 정리했습니다. 바나바잎 코로솔산 1.3mg·아연·프로바이오틱스 1억 CFU 복합 분말 제품입니다.",
    description: "",
    heroDescription: "식후 혈당 관리와 장 건강을 함께 챙기고 싶다면 이 제품을 볼 수 있어요. jw중외제약의 혈당 바나바 유산균은 검은색·갈색 입자가 섞인 노란 하얀색 분말에 바나바잎 추출물(코로솔산 1.3mg)과 프로바이오틱스 1억 CFU, 아연 2.55mg을 담았어요. 어린이·임산부·수유부는 섭취할 수 없는 성인 전용 제품이에요.",
    datePublished: "2026-04-04",
    dateModified: "2026-04-04",
    sections: [
      {
        title: "jw중외제약 혈당 바나바 유산균 Pro 성분은 무엇인가요?",
        content: "고유의 향미를 가지며 이미·이취가 없는 검은색 또는 갈색의 입자가 있는 노란 하얀색 분말이에요. 1포(2g)에 바나바잎 추출물(코로솔산 1.3mg), 프로바이오틱스 1억 CFU, 아연 2.55mg이 들어 있어요. 검은색·갈색 입자가 바나바잎 추출물에서 비롯된 색상으로, 독특한 외관이 특징이에요.",
        ingredients: [
          { type: "주성분", name: "프로바이오틱스", amount: "100억 CFU/5,000mg당", role: "유산균 증식·유해균 억제, 배변활동 원활, 장 건강" },
          { type: "주성분", name: "프락토올리고당", amount: "3,000mg/5,000mg당", role: "프리바이오틱스로 유익균 성장 지원" },
          { type: "주성분", name: "아연", amount: "8.5mg/5,000mg당", role: "정상적인 면역기능, 정상적인 세포분열" },
        ],
      },
      {
        title: "jw중외제약 혈당 바나바 유산균 Pro 효과가 나타나려면 얼마나 걸리나요?",
        content: "바나바잎 추출물은 식후 혈당상승 억제에 도움을 줄 수 있어요. 프로바이오틱스는 유산균 증식 및 유해균 억제, 배변활동 원활, 장 건강에 도움을 줄 수 있어요. 아연은 정상적인 면역기능과 세포분열에 필요한 성분이에요. 혈당 관리와 장 건강, 면역을 함께 지원하는 구성이에요."
      },
      {
        title: "jw중외제약 혈당 바나바 유산균 Pro 복용법은 어떻게 되나요?",
        content: "1일 1회, 1회 1포를 직접 섭취하면 돼요. 물 없이 직접 섭취하는 방식이에요."
      },
      {
        title: "jw중외제약 혈당 바나바 유산균 Pro 주의사항은 무엇인가요?",
        content: "알레르기 체질은 성분을 확인한 후 섭취하세요. 바나바잎 추출물이 들어 있어 어린이·임산부·수유부는 섭취를 피해야 해요. 이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 프로바이오틱스 관련해서는 질환이 있거나 의약품 복용 시 전문가와 상담하고, 알레르기 체질은 과민반응에 주의하세요. 어린이가 임의로 섭취하지 않도록 지도하세요."
      },
      {
        title: "jw중외제약 혈당 바나바 유산균 Pro 보관할 때 주의할 점은 무엇인가요?",
        content: "고온다습한 곳이나 직사광선을 피해 서늘한 곳에 보관하세요. 유통기한은 제조일로부터 24개월이에요. 혈당 관련 제품 중 유통기한이 24개월로 비교적 긴 편이에요."
      }
    ],
    products: [],
    faq: [
      {
        question: "바나바잎 추출물이 혈당을 낮춰주는 건가요?",
        answer: "바나바잎 추출물은 식후 혈당상승 억제에 도움을 줄 수 있는 기능성 원료예요. 혈당을 직접 낮추는 의약품이 아니라 식품 기능성 성분이에요. 당뇨 치료제를 대체할 수 없으니 혈당 문제가 있다면 반드시 의사와 상담하세요."
      },
      {
        question: "임산부가 먹으면 안 되는 이유가 있나요?",
        answer: "바나바잎 추출물은 임산부·수유부에게 섭취를 피하도록 규정된 성분이에요. 태아나 영아에 미치는 영향이 충분히 연구되지 않았기 때문이에요. 임신 중이거나 수유 중이라면 반드시 피해야 해요."
      },
      {
        question: "이상 반응이 생기면 어떻게 해야 하나요?",
        answer: "이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 알레르기나 소화 이상이 나타나면 성분에 대한 과민반응일 수 있어요."
      }
    ]
  },

  // 6. 가르시니아다이어트프로바이오틱스유산균
  {
    slug: "가르시니아다이어트프로바이오틱스유산균",
    categorySlug: "유산균",
    title: "가르시니아 다이어트 프로바이오틱스 유산균 성분 효과 | 체지방 감소 복용법 총정리",
    h1: "가르시니아 다이어트 프로바이오틱스 유산균 성분 효과 | 체지방 감소 복용법 총정리",
    metaDescription: "가르시니아 다이어트 프로바이오틱스 유산균의 성분, 효과, 복용법, 주의사항을 정리했습니다. 가르시니아캄보지아 HCA 750mg과 프로바이오틱스 1억 CFU가 담긴 연한 갈색 분말 포 제품입니다.",
    description: "",
    heroDescription: "체지방 감소 기능성과 장 건강을 함께 담은 유산균을 찾는다면 이 제품이 맞을 수 있어요. 가르시니아캄보지아 추출물(HCA 750mg)과 프로바이오틱스 1억 CFU를 3g 포의 연한 갈색 분말에 담았어요. 어린이·임산부·수유부는 섭취할 수 없는 성인 전용 제품이에요.",
    datePublished: "2026-04-04",
    dateModified: "2026-04-04",
    sections: [
      {
        title: "가르시니아 다이어트 프로바이오틱스 유산균 성분은 무엇인가요?",
        content: "이미·이취가 없고 고유의 향미를 가진 연한 갈색 분말이에요. 1포(3g)에 가르시니아캄보지아 추출물(HCA 750mg)과 프로바이오틱스 1억 CFU가 들어 있어요. 체지방 감소 기능성 성분과 장 건강 성분을 함께 담은 구성이에요.",
        ingredients: [
          { type: "주성분", name: "바나바잎 추출물(코로솔산)", amount: "1.3mg/2g당", role: "기능성 원료" },
          { type: "주성분", name: "프로바이오틱스", amount: "1억 CFU/2g당", role: "유산균 증식·유해균 억제, 배변활동 원활, 장 건강" },
          { type: "주성분", name: "아연", amount: "2.55mg/2g당", role: "정상적인 면역기능, 정상적인 세포분열" },
        ],
      },
      {
        title: "가르시니아 다이어트 프로바이오틱스 유산균 효과가 나타나려면 얼마나 걸리나요?",
        content: "가르시니아캄보지아 추출물은 탄수화물이 지방으로 합성되는 것을 억제하여 체지방 감소에 도움을 줄 수 있어요. 프로바이오틱스는 유산균 증식 및 유해균 억제, 배변활동 원활, 장 건강에 도움을 줄 수 있어요. 체지방 감소와 장 건강 지원을 함께 하는 구성이에요."
      },
      {
        title: "가르시니아 다이어트 프로바이오틱스 유산균 복용법은 어떻게 되나요?",
        content: "1일 1회, 1회 1포를 섭취하면 돼요. 별도 물과 함께 또는 직접 섭취에 대한 명시가 없으니 물과 함께 드시는 게 편해요."
      },
      {
        title: "가르시니아 다이어트 프로바이오틱스 유산균 주의사항은 무엇인가요?",
        content: "가르시니아캄보지아 추출물이 들어 있어 어린이·임산부·수유부는 섭취를 피해야 해요. 간·신장·심장 질환, 알레르기, 천식이 있거나 의약품을 복용 중이라면 전문가와 상담하세요. 이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 프로바이오틱스 관련해서도 질환이 있거나 의약품 복용 시 전문가와 상담하고, 알레르기 체질은 과민반응에 주의하세요."
      },
      {
        title: "가르시니아 다이어트 프로바이오틱스 유산균 보관할 때 주의할 점은 무엇인가요?",
        content: "고온다습하고 직사광선이 드는 곳을 피해 서늘한 곳에 보관하세요. 유통기한은 제조일로부터 18개월이에요."
      }
    ],
    products: [],
    faq: [
      {
        question: "가르시니아 제품을 간 질환이 있을 때 먹으면 안 되는 이유가 있나요?",
        answer: "가르시니아캄보지아 추출물 관련 이상사례 보고 중 간 관련 이상이 있어요. 간 질환이 있는 분은 반드시 섭취 전 전문가와 상담해야 해요."
      },
      {
        question: "임산부가 먹으면 안 되나요?",
        answer: "가르시니아캄보지아 추출물은 임산부·수유부에게 섭취를 피하도록 규정된 성분이에요. 임신 중이거나 수유 중이라면 반드시 피해야 해요."
      },
      {
        question: "이상사례가 생기면 어떻게 해야 하나요?",
        answer: "이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 소화 이상이나 알레르기 증상이 나타나면 빠르게 전문가에게 확인받는 게 좋아요."
      }
    ]
  },

  // 7. 3-스텝활성유산균
  {
    slug: "3-스텝활성유산균",
    categorySlug: "유산균",
    title: "3-스텝 활성 유산균 성분 효과 | 0.4g 초소형 캡슐 복용법 주의사항 총정리",
    h1: "3-스텝 활성 유산균 성분 효과 | 0.4g 초소형 캡슐 복용법 주의사항 총정리",
    metaDescription: "3-스텝 활성 유산균의 성분, 효과, 복용법, 주의사항을 정리했습니다. 5.43억 CFU를 담은 0.4g 초소형 백색 경질캡슐 제품입니다.",
    description: "",
    heroDescription: "아주 작은 캡슐 하나에 유산균을 담은 제품을 찾는다면 이 제품이 맞을 수 있어요. 3-스텝 활성 유산균은 노란 하얀색 분말을 담은 백색 경질캡슐 단 0.4g에 5.43억 CFU를 담은 초소형 캡슐이에요. 임산부·수유부는 섭취에 주의가 필요하고, 개봉 후 밀봉 보관이 중요한 제품이에요.",
    datePublished: "2026-04-04",
    dateModified: "2026-04-04",
    sections: [
      {
        title: "3-스텝 활성 유산균 성분은 무엇인가요?",
        content: "노란 하얀색 분말을 함유한 백색 경질캡슐이에요. 1캡슐(0.4g)에 프로바이오틱스 5.43억 CFU가 들어 있어요. 0.4g이라는 매우 소형 캡슐에 5억 이상의 유산균을 담은 게 특징이에요. 1억~10억 CFU 범위 내의 중저 함량이에요.",
        ingredients: [
          { type: "주성분", name: "프로바이오틱스", amount: "543,000,000 CFU/0.4 g", role: "유산균 증식·유해균 억제, 배변활동 원활, 장 건강" },
        ]
      },
      {
        title: "3-스텝 활성 유산균 효과가 나타나려면 얼마나 걸리나요?",
        content: "프로바이오틱스는 유산균 증식 및 유해균 억제, 배변활동 원활에 도움을 줄 수 있어요. 0.4g 소형 캡슐이라 삼키기 부담이 적으면서도 5억 이상의 유산균을 섭취할 수 있는 구성이에요."
      },
      {
        title: "3-스텝 활성 유산균 복용법은 어떻게 되나요?",
        content: "1일 1회, 1회 1캡슐(0.4g)을 충분한 물과 함께 섭취하면 돼요. 캡슐이 매우 작아서 삼키기 편하지만, 충분한 물과 함께 드세요. 캡슐 개봉 시 포장재에 의해 상처를 입을 수 있으니 주의하세요."
      },
      {
        title: "3-스텝 활성 유산균 주의사항은 무엇인가요?",
        content: "임산부와 수유부는 섭취에 주의해야 해요. 특정 성분에 알레르기가 있는 분은 원료명을 확인한 후 섭취하세요. 개봉 또는 섭취 시 포장재에 의해 상처를 입을 수 있으니 주의하세요."
      },
      {
        title: "3-스텝 활성 유산균 보관할 때 주의할 점은 무엇인가요?",
        content: "직사광선을 피해 서늘한 곳에 보관하세요. 제품 개봉 후에는 흡습되지 않게 밀봉하여 보관해야 해요. 캡슐이 습기에 노출되면 변형될 수 있어요. 유통기한은 제조일부터 18개월이에요."
      }
    ],
    products: [],
    faq: [
      {
        question: "0.4g 캡슐이 너무 작으면 효과가 덜한가요?",
        answer: "캡슐 크기보다 유산균 CFU 수가 효과에 더 중요해요. 0.4g에 5.43억 CFU가 들어 있어 충분한 함량을 가지고 있어요. 삼키기 편한 소형 캡슐을 선호하는 분께 맞는 제품이에요."
      },
      {
        question: "임산부는 왜 주의해야 하나요?",
        answer: "이 제품은 임산부·수유부에게 '섭취를 피하라'가 아닌 '섭취에 주의하라'고 안내하고 있어요. 임신 중이거나 수유 중이라면 섭취 전 반드시 의사나 약사와 상담하는 게 안전해요."
      },
      {
        question: "개봉 후 밀봉을 안 하면 어떻게 되나요?",
        answer: "개봉 후 밀봉하지 않으면 습기가 들어가 캡슐이 변형되거나 유산균 생존율이 낮아질 수 있어요. 개봉 후에는 반드시 밀봉하여 보관하세요."
      }
    ]
  },

  // 8. 365와이Y케어유산균
  {
    slug: "365와이Y케어유산균",
    categorySlug: "유산균",
    title: "365 와이Y 케어 유산균 성분 효과 | 진분홍색 알갱이 셀렌·아연 캡슐 복용법 총정리",
    h1: "365 와이Y 케어 유산균 성분 효과 | 진분홍색 알갱이 셀렌·아연 캡슐 복용법 총정리",
    metaDescription: "365 와이Y 케어 유산균의 성분, 효과, 복용법, 주의사항을 정리했습니다. 10억 CFU에 셀렌 22.5μg·아연 4.25mg이 담긴 진분홍색 알갱이 분말의 0.35g 초소형 투명 캡슐입니다.",
    description: "",
    heroDescription: "0.35g 초소형 캡슐에 10억 유산균과 셀렌·아연을 담은 제품을 찾는다면 이 제품이 맞을 수 있어요. 진분홍색 알갱이가 들어 있는 분말의 투명 경질캡슐로, 0.35g이라는 매우 작은 크기에 10억 CFU, 셀렌 22.5μg, 아연 4.25mg이 담겨 있어요.",
    datePublished: "2026-04-04",
    dateModified: "2026-04-04",
    sections: [
      {
        title: "365 와이Y 케어 유산균 성분은 무엇인가요?",
        content: "알갱이가 있는 진분홍색 분말을 함유한 투명 경질캡슐이에요. 1캡슐(0.35g)에 프로바이오틱스 10억 CFU, 셀렌 22.5μg, 아연 4.25mg이 들어 있어요. 0.35g이라는 극소형 캡슐 안에 3가지 기능성 성분을 담아 삼키기 부담 없는 구성이에요.",
        ingredients: [
          { type: "주성분", name: "프로바이오틱스", amount: "5.43억 CFU/0.4g당", role: "유산균 증식·유해균 억제, 배변활동 원활, 장 건강" },
        ],
      },
      {
        title: "365 와이Y 케어 유산균 효과가 나타나려면 얼마나 걸리나요?",
        content: "셀렌은 유해산소로부터 세포를 보호하는 데 필요해요. 아연은 정상적인 면역기능과 세포분열에 필요해요. 프로바이오틱스는 유산균 증식 및 유해균 억제, 배변활동 원활, 장 건강에 도움을 줄 수 있어요. 소형 캡슐 하나에 장 건강·항산화·면역 3가지를 지원하는 구성이에요."
      },
      {
        title: "365 와이Y 케어 유산균 복용법은 어떻게 되나요?",
        content: "1일 1회, 1회 1캡슐씩 충분한 물과 함께 섭취하면 돼요. 0.35g 초소형 캡슐이라 물 없이도 삼킬 수 있지만, 충분한 물과 함께 드세요."
      },
      {
        title: "365 와이Y 케어 유산균 주의사항은 무엇인가요?",
        content: "질환이 있거나 의약품을 복용 중이라면 전문가와 상담하세요. 알레르기 체질은 과민반응이 나타날 수 있어요. 어린이가 임의로 섭취하지 않도록 지도하고, 이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요."
      },
      {
        title: "365 와이Y 케어 유산균 보관할 때 주의할 점은 무엇인가요?",
        content: "습기가 적고 직사광선을 피해 서늘한 곳에 보관하고, 어린이 손에 닿지 않도록 주의하세요. 유통기한은 제조일로부터 18개월이에요."
      }
    ],
    products: [],
    faq: [
      {
        question: "0.35g 캡슐에 10억 유산균이 다 들어가나요?",
        answer: "동결건조 기술로 고농도의 균주를 소형 캡슐에 압축하는 게 가능해요. 0.35g 초소형 캡슐에 10억 CFU가 보장되는 제품이에요. 보관 조건(습기·온도)을 잘 지켜야 유산균이 유통기한까지 보장돼요."
      },
      {
        question: "셀렌과 아연을 같이 먹으면 어떤 점이 좋은가요?",
        answer: "셀렌은 항산화 작용으로 세포를 유해산소로부터 보호하고, 아연은 면역기능에 필요해요. 두 성분을 함께 섭취하면 항산화와 면역 두 방향에서 지원을 받을 수 있어요."
      },
      {
        question: "이상 반응이 생기면 어떻게 해야 하나요?",
        answer: "이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 소화 이상이나 알레르기 반응이 나타나면 성분에 대한 과민반응일 수 있어요."
      }
    ]
  },

  // 9. 50억보장장건강해질유산균
  {
    slug: "50억보장장건강해질유산균",
    categorySlug: "유산균",
    title: "50억보장 장 건강해질 유산균 성분 효과 | 분홍색 분말 아연·셀레늄 캡슐 복용법 총정리",
    h1: "50억보장 장 건강해질 유산균 성분 효과 | 분홍색 분말 아연·셀레늄 캡슐 복용법 총정리",
    metaDescription: "50억보장 장 건강해질 유산균의 성분, 효과, 복용법, 주의사항을 정리했습니다. 50억 CFU에 아연 2.55mg·셀레늄 16.5μg이 담긴 분홍색 분말의 500mg 투명 경질캡슐입니다.",
    description: "",
    heroDescription: "50억 CFU에 아연과 셀레늄까지 담은 캡슐을 찾는다면 이 제품이 맞을 수 있어요. 분홍색 분말을 담은 투명 경질캡슐 500mg에 50억 CFU, 아연 2.55mg, 셀레늄 16.5μg이 들어 있어요. 메디오젠이 제조한 50억 중함량 복합 구성 제품이에요.",
    datePublished: "2026-04-04",
    dateModified: "2026-04-04",
    sections: [
      {
        title: "50억보장 장 건강해질 유산균 성분은 무엇인가요?",
        content: "고유의 향미가 있고 이미·이취가 없는 분홍색 분말을 함유한 투명 경질캡슐이에요. 1캡슐(500mg)에 프로바이오틱스 50억 CFU, 아연 2.55mg, 셀레늄 16.5μg이 들어 있어요. 중함량(50억)에 항산화(셀레늄)와 면역(아연)까지 더한 3중 구성이에요.",
        ingredients: [
          { type: "주성분", name: "가르시니아캄보지아 추출물(HCA)", amount: "750mg/3g당", role: "탄수화물이 지방으로 합성되는 것을 억제하여 체지방 감소에 도움" },
          { type: "주성분", name: "프로바이오틱스", amount: "1억 CFU/3g당", role: "유산균 증식·유해균 억제, 배변활동 원활, 장 건강" },
          { type: "주성분", name: "비타민C", amount: "50mg/3g당", role: "기능성 원료" },
        ],
      },
      {
        title: "50억보장 장 건강해질 유산균 효과가 나타나려면 얼마나 걸리나요?",
        content: "프로바이오틱스는 유산균 증식 및 유해균 억제, 배변활동 원활, 장 건강에 도움을 줄 수 있어요. 아연은 정상적인 면역기능과 세포분열에 필요해요. 셀레늄은 유해산소로부터 세포를 보호하는 데 필요한 성분이에요. 장 건강, 면역, 항산화를 함께 지원하는 50억 중함량 제품이에요."
      },
      {
        title: "50억보장 장 건강해질 유산균 복용법은 어떻게 되나요?",
        content: "1일 1회, 1회 1캡슐을 물과 함께 섭취하면 돼요. 500mg 캡슐로 일반적인 크기이므로 물과 함께 삼키면 돼요."
      },
      {
        title: "50억보장 장 건강해질 유산균 주의사항은 무엇인가요?",
        content: "질환이 있거나 의약품을 복용 중이라면 전문가와 상담하세요. 알레르기 체질은 과민반응이 나타날 수 있어요. 어린이가 임의로 섭취하지 않도록 지도하고, 이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요."
      },
      {
        title: "50억보장 장 건강해질 유산균 보관할 때 주의할 점은 무엇인가요?",
        content: "직사광선 및 고온다습한 곳을 피해 서늘한 곳에 보관하세요. 유통기한은 제조일로부터 18개월이에요. 캡슐 제형이라 습기 차단이 중요해요."
      }
    ],
    products: [],
    faq: [
      {
        question: "50억 CFU면 어떤 분에게 맞나요?",
        answer: "10억에서 함량을 높이고 싶지만 100억은 부담스러운 분이나, 중간 함량을 원하는 분께 맞는 제품이에요. 50억은 중함량대에 해당해요."
      },
      {
        question: "분홍색 분말이 들어 있는 이유가 있나요?",
        answer: "소스 데이터에는 분홍색 색상의 원인이 별도로 명시되어 있지 않아요. 제품의 고유한 색택으로, 품질과는 무관해요."
      },
      {
        question: "이상 반응이 생기면 어떻게 해야 하나요?",
        answer: "이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 알레르기나 소화 이상이 나타나면 성분에 대한 과민반응일 수 있어요."
      }
    ]
  },

  // 10. 가벼운다이어트유산균
  {
    slug: "가벼운다이어트유산균",
    categorySlug: "유산균",
    title: "가벼운 다이어트 유산균 성분 효과 | 가르시니아·비타민C 복합 복용법 주의사항 총정리",
    h1: "가벼운 다이어트 유산균 성분 효과 | 가르시니아·비타민C 복합 복용법 주의사항 총정리",
    metaDescription: "가벼운 다이어트 유산균의 성분, 효과, 복용법, 주의사항을 정리했습니다. 가르시니아캄보지아 HCA 750mg·비타민C 50mg·프로바이오틱스 1억 CFU가 담긴 점박이 노란 하얀색 분말입니다.",
    description: "",
    heroDescription: "체지방 감소와 항산화, 장 건강을 한 포에 담고 싶다면 이 제품을 볼 수 있어요. 가르시니아캄보지아 추출물(HCA 750mg)에 비타민C 50mg과 프로바이오틱스 1억 CFU를 더한 한미양행 제품이에요. 점박이가 있는 노란 하얀색 분말로 물 없이 직접 섭취하는 방식이에요. 어린이·임산부·수유부, 남성의 과다섭취는 피해야 하는 제품이에요.",
    datePublished: "2026-04-04",
    dateModified: "2026-04-04",
    sections: [
      {
        title: "가벼운 다이어트 유산균 성분은 무엇인가요?",
        content: "고유의 향미를 지니고 이미·이취가 없는 점박이가 있는 노란 하얀색 또는 흰 노란색 분말이에요. 1포(3g)에 가르시니아캄보지아 추출물(HCA 750mg), 프로바이오틱스 1억 CFU, 비타민C 50mg이 들어 있어요. 가르시니아캄보지아에 비타민C(항산화)를 더한 게 다른 가르시니아 유산균 제품과 차별화되는 포인트예요.",
        ingredients: [
          { type: "주성분", name: "프로바이오틱스", amount: "100,000,000(1억", role: "유산균 증식·유해균 억제, 배변활동 원활, 장 건강" },
        ]
      },
      {
        title: "가벼운 다이어트 유산균 효과가 나타나려면 얼마나 걸리나요?",
        content: "가르시니아캄보지아 추출물은 탄수화물이 지방으로 합성되는 것을 억제하여 체지방 감소에 도움을 줄 수 있어요. 프로바이오틱스는 유산균 증식 및 유해균 억제, 배변활동 원활, 장 건강에 도움을 줄 수 있어요. 비타민C는 결합조직 형성과 기능 유지에 필요하고, 철의 흡수에 필요하며, 항산화 작용으로 유해산소로부터 세포를 보호하는 데 필요한 성분이에요."
      },
      {
        title: "가벼운 다이어트 유산균 복용법은 어떻게 되나요?",
        content: "1일 1회, 1회 1포를 직접 섭취하면 돼요. 물 없이 직접 섭취하는 방식이에요. 3g 포라 물에 타도 섭취할 수 있어요."
      },
      {
        title: "가벼운 다이어트 유산균 주의사항은 무엇인가요?",
        content: "알레르기 체질은 성분을 확인한 후 섭취하세요. 가르시니아캄보지아 추출물이 들어 있어 어린이·임산부·수유부는 섭취를 피해야 해요. 간·신장·심장 질환, 알레르기, 천식이 있거나 의약품을 복용 중이라면 전문가와 상담하세요. 남성의 경우 과다 섭취를 피해야 해요. 비타민C 관련해서는 과량 섭취하지 않도록 주의하고, 신장질환이 있으면 섭취 전 전문가와 상담하세요. 이상사례 발생 시 즉시 섭취를 중단하세요."
      },
      {
        title: "가벼운 다이어트 유산균 보관할 때 주의할 점은 무엇인가요?",
        content: "고온다습하고 직사광선이 드는 곳을 피해 서늘한 곳에 보관하세요. 유통기한은 제조일로부터 18개월이에요."
      }
    ],
    products: [],
    faq: [
      {
        question: "비타민C가 들어 있으면 가르시니아와 시너지가 있나요?",
        answer: "비타민C는 항산화 작용과 결합조직 형성에 필요한 성분이에요. 가르시니아와 직접적인 시너지보다는 체지방 관리 중 항산화 기능을 보완하는 역할을 해요."
      },
      {
        question: "신장질환이 있으면 비타민C 때문에 주의해야 하나요?",
        answer: "비타민C를 과량 섭취하면 신장에 옥살산 결석이 생길 위험이 있어요. 이 제품은 1포에 50mg으로 과량은 아니지만, 신장질환이 있는 분은 섭취 전 전문가와 상담하는 게 안전해요."
      },
      {
        question: "이상사례가 생기면 어떻게 해야 하나요?",
        answer: "이상사례 발생 시 즉시 섭취를 중단하고 전문가와 상담하세요. 특히 간·신장 관련 증상이나 알레르기 반응이 생기면 빠르게 전문가에게 확인받으세요."
      }
    ]
  }
];
