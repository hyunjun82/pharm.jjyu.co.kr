import { Category } from "@/lib/types";

export const categories: Category[] = [
  {
    name: "탈모약",
    slug: "탈모",
    icon: "💊",
    description: "미녹시딜, 피나스테리드 등 탈모 치료제",
    count: 8,
  },
  {
    name: "연고",
    slug: "연고",
    icon: "🩹",
    description: "마데카솔, 후시딘 등 상처·피부 치료 연고",
    count: 12,
  },
  {
    name: "감기약",
    slug: "감기",
    icon: "🤧",
    description: "타이레놀콜드, 판콜에이 등 종합감기약",
    count: 10,
  },
  {
    name: "진통제",
    slug: "진통제",
    icon: "💉",
    description: "타이레놀, 이부프로펜 등 통증 완화제",
    count: 8,
  },
  {
    name: "무좀약",
    slug: "무좀",
    icon: "🦶",
    description: "라미실, 카네스텐 등 무좀 치료제",
    count: 6,
  },
  {
    name: "설사약",
    slug: "설사",
    icon: "💧",
    description: "스멕타, 정로환 등 설사·장염 치료제",
    count: 5,
  },
  {
    name: "소화제",
    slug: "소화제",
    icon: "🫗",
    description: "베아제, 훼스탈 등 소화 촉진제",
    count: 7,
  },
  {
    name: "안약",
    slug: "안약",
    icon: "👁️",
    description: "시크린, 인공눈물 등 안과용 의약품",
    count: 5,
  },
  {
    name: "구강/잇몸",
    slug: "구강",
    icon: "🦷",
    description: "인사돌, 오라메디 등 구강·잇몸 치료제",
    count: 8,
  },
  {
    name: "파스/진통겔",
    slug: "파스",
    icon: "🩹",
    description: "케토톱, 볼타렌 등 근육·관절 소염진통제",
    count: 10,
  },
  {
    name: "비타민/영양제",
    slug: "영양제",
    icon: "🌿",
    description: "비타민C, 오메가3, 루테인 등 비타민·영양제",
    count: 10,
  },
  {
    name: "여성건강/생리통",
    slug: "여성건강",
    icon: "🌸",
    description: "이지엔6이브, 부스코판 등 생리통·여성 건강 의약품",
    count: 9,
  },
  {
    name: "외상소독",
    slug: "외상소독",
    icon: "🩺",
    description: "베타딘, 리바놀, 과산화수소 등 상처 소독·외상 처치 의약품",
    count: 9,
  },
  {
    name: "두드러기/피부로션",
    slug: "두드러기",
    icon: "🧴",
    description: "멘소래담, 락티케어HC, 칼라민 등 피부가려움·두드러기 완화 제품",
    count: 9,
  },
  {
    name: "구충제",
    slug: "구충제",
    icon: "🪱",
    description: "젤콤, 알싹, 대웅 알벤다졸 등 회충·요충 구충제",
    count: 7,
  },
  {
    name: "변비약",
    slug: "변비",
    icon: "🌀",
    description: "둘코락스, 마그밀, 아락실 등 변비 완화·배변 촉진 의약품",
    count: 6,
  },
  {
    name: "알레르기/항히스타민",
    slug: "알레르기",
    icon: "🌺",
    description: "지르텍, 알레그라 등 알레르기성 비염·두드러기 항히스타민제",
    count: 6,
  },
  {
    name: "위식도역류/제산제",
    slug: "제산제",
    icon: "🫀",
    description: "겔포스, 알마겔 등 위산과다·속쓰림·역류성 식도염 완화 제산제",
    count: 9,
  },
  {
    name: "상처/드레싱",
    slug: "상처관리",
    icon: "🩹",
    description: "메디폼, 케어리브, 대일밴드 등 상처 보호·습윤드레싱·티눈 관리 용품",
    count: 10,
  },
];
