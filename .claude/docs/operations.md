...
## 4. 타이틀 공식 (v3 — master-quality.template.md가 우선)

구 공식(가격 숫자 "{minPrice}원~" 포함, 파이프 나열형)은 **폐기**. 신규 규칙:

- 제품명 선두 + 검색 1위 의도 후킹 질문/문장 + "가격|최저가" 단어(숫자 금지)
- 키워드 3~5개 자연 연결, 파이프 최대 1회, 28~40자
- 못 지키는 약속 금지("실시간 비교" 등), 같은 성분군 내 패턴 중복 금지
- 6패턴(비교/정의/PAA/후킹/가격/대안): .claude/templates/master-quality.template.md 참조
