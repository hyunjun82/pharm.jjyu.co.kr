...
- **완료의 정의 = 라이브 반영 + 육안 확인 (2026-07-02 신설)**: 글 작성·게이트 통과는 "완료"가 아니다. 부분배포 후 라이브 URL에서 신버전 확인까지 끝나야 완료. 6월 리라이트 69편이 배포 누락으로 한 달간 라이브 미반영됐던 사고의 재발 방지.
- **한 파일의 생성 주인은 하나**: 같은 산출물(sitemap.xml 등)을 두 스크립트가 만들면 안 됨 (2026-07-02 사이트맵 슬래시 회귀 사고 — app/sitemap.ts와 generate-sitemap.js 이중 생성이 원인).
...
6. **소스 무결성 우선 (2026-07-02 신설 — 536페이지 오염 사고)**: 같은 품목번호(itemSeq)를 공유하거나 슬러그 제형(정/캡슐/액)과 소스 itemName 제형이 다르면 NEEDS_REFETCH 자동 차단. `reports/소스오염-품목번호공유-2026-07-02.md`의 수리 완료 전까지 해당 슬러그 리라이트 금지.
7. **배치 전 큐 신선도 검사 의무**: rewrite-queue는 실제 데이터의 dateModified와 대조 후 사용 (2026-07-02 실측 — 큐의 79%가 이미 리라이트된 글이었음). 최신 리라이트 기준일 이후 수정된 글은 batch-done 처리.
...
| 작성 후 | 검증기 26규칙 | validate-article.js (타이틀·숫자 화이트리스트·복붙·어미 단조·섹션 300자 + **2026-07-02 신설: B17 벽글(문단<2), B18 상투 공감 서론, B19 출처 전용 섹션**) |
