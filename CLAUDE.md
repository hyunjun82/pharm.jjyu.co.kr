...
Q→내용 연결 (섹션 순서는 고정, Q는 내용 깊이/방식만 결정):
- Q1 답 → 서론 첫 문장 (독자 상황 공감)
- Q2 답 → 어느 섹션을 가장 두껍게 쓸지 (강조점 결정, 순서 변경 금지)
- Q3 답 → 각 섹션 내 예외/주의사항 깊이
- Q4 답 → 각 섹션 전달 방식 (표, 텍스트, 경고박스 등)

Q2 답에 따라 강조점이 달라진다 (순서는 그대로):
- "비교해서 고른다" → 성분 섹션에 비교표 추가, 텍스트 짧게
- "용량 확인하고 복용한다" → 복용법 섹션을 가장 두껍게
- "부작용 판단한다" → 부작용 섹션을 가장 두껍게
- "대체약을 찾는다" → 성분 섹션에 비교표 + 내부링크
...
모든 spoke 글은 아래 순서를 반드시 따른다. Q2는 강조점만 결정하고 순서는 바꾸지 않는다.

### 필수 섹션 (고정 순서)

| 순서 | 섹션 제목 형식 | 필수 요소 | 비고 |
|------|---------------|----------|------|
| 1 | `{의약품명} 성분 분석` | ✅ `ingredients` 배열 필수 | - |
| 2 | `{의약품명} 효능과 효과` | - | - |
| 3 | `{의약품명} 올바른 {사용법/복용법}` | - | 연고류→사용법, 복용약→복용법 |
| 4 | `{의약품명} 최저가 가격 비교` | - | ⚠️ 발키리 딥링크 있을 때만 포함 |
| 5 | `{의약품명} 부작용` | - | - |
| 6 | `{의약품명} 주의사항` | - | - |
| 7 | `{의약품명} 보관법` | - | - |

- 가격 섹션이 없으면 sections는 6개, 있으면 7개
- 순서 변경 절대 금지 — Q2는 "어느 섹션을 두껍게 쓸지"만 결정
...
| 소스 JSON 필드 | 글 섹션 (고정 순서) |
|---------------|--------|
| `itemName` 괄호 안 성분 | 1번: 성분 분석 |
| `efcyQesitm` (효능효과) | 2번: 효능과 효과 |
| `useMethodQesitm` (용법용량) | 3번: 올바른 사용법/복용법 |
| (발키리 딥링크 데이터) | 4번: 최저가 가격 비교 (있을 때만) |
| `seQesitm` (부작용) | 5번: 부작용 |
| `atpnQesitm` (주의사항) | 6번: 주의사항 |
| `depositMethodQesitm` (보관법) | 7번: 보관법 |
