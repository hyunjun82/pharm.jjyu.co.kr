...
## 8대 원칙 (글 쓰기 전에 반드시 체화할 것)

> **글을 다 쓰고 검증에서 오류를 잡는 게 아니라, 처음부터 오류 없이 쓰는 것이 이 시스템의 목표다.**
> 아래 8가지를 이해하지 못한 상태에서 글을 시작하면 안 된다.
> 검증 스크립트(verify-wiki-quality.js)는 안전망일 뿐, 주력이 아니다.

| # | 원칙 | 핵심 질문 | 위반 시 |
|---|------|----------|--------|
| 1 | **스토리텔링** | 섹션 6개, 각 3문단 이상, 성분→효능→용법→부작용→주의→보관 자연스러운 흐름 | 정보 나열형 글이 됨 |
| 2 | **기승전결** | title=h1, hero=공감형("~고민이죠?"), desc=가치제안형, hero≠desc | 검색 클릭률 하락 |
| 3 | **문체** | 구어체("~해요") 통일, 문어체 금지(metaDescription 예외), AI냄새 단어 금지 | 딱딱한 설명서 글 |
| 4 | **손석희+유시민** | 첫 문장=결론, 숫자에 소스 근거, 전문용어 괄호 설명, 반박 불가 논리 | 신뢰도 0 |
| 5 | **가독성** | 80자 이내, 전문용어 쉬운 설명, 20대~80대 이해 가능 | 이탈률 증가 |
| 6 | **문제해결 100%** | "이 약 어떻게 먹어야 해?" 완전 해결, 용법용량·주의사항 정확 | "그래서 뭘 하라고?" |
| 7 | **오차 제로** | source-data JSON에 없는 효능·부작용 추가 금지, 숫자 정확 일치 | 의약품 오정보 = 위험 |
| 8 | **출처 명시** | 모든 팩트는 source-data/{slug}.json (e약은요 API) 기반, AI 지식 사용 금지 | 근거 없는 글 = 삭제 대상 |

### 자가검증 7항목 (섹션 하나 쓸 때마다 체크)

글을 쓰면서 매 섹션마다 아래 7개를 머릿속으로 확인한다. Stop 훅이 자동 검증하지만, 거기서 걸리면 = 원칙을 놓친 것이다.

| # | 체크 | 확인 방법 |
|---|------|----------|
| 1 | 첫 문장이 결론인가? | "알아보겠~", "살펴보겠~" 으로 시작하면 실패 |
| 2 | 3문단 이상인가? | `\n\n`으로 구분된 문단 수 세기 |
| 3 | 숫자가 소스와 일치하는가? | "1일 N회", "만 N세" → source-data JSON 대조 |
| 4 | 문어체가 섞였는가? | "~합니다", "~입니다" 검색 → 0건이어야 함 |
| 5 | AI냄새 단어가 있는가? | "또한", "다양한", "매우 중요", "결론적으로" → 0건 |
| 6 | hero와 desc가 다른가? | 90% 이상 겹치면 분리 필요 |
| 7 | 소스에 없는 정보를 추가했는가? | source-data에 없는 효능·부작용 → 삭제 |
...
### 수동 검증 항목 (8대 원칙 기반)
- [ ] title과 h1이 동일한가 (원칙2)
- [ ] 구어체 ("~해요") 통일, 문어체 0건인가 (원칙3)
- [ ] AI냄새 단어("또한", "다양한", "매우 중요") 0건인가 (원칙3)
- [ ] 섹션 6개, 각 3문단 이상, FAQ 3개인가 (원칙1)
- [ ] 각 섹션 첫 문장이 결론인가 — "알아보겠~" 금지 (원칙4)
- [ ] hero=공감형, desc=가치제안형, 둘이 90% 이상 겹치지 않는가 (원칙2)
- [ ] 첫 번째 섹션에 ingredients 배열이 있는가 (원칙1)
- [ ] 숫자(1일N회, 만N세)가 source-data JSON과 일치하는가 (원칙7)
- [ ] source-data에 없는 효능·부작용을 추가하지 않았는가 (원칙8)
- [ ] 전문용어 뒤 괄호 설명이 있는가 (원칙5)
- [ ] barkiryProductId가 200 응답인가
- [ ] hub spokes에 새 글이 추가되었는가
- [ ] products에 새 제품이 추가되었는가

> **Stop 훅(stop-quality-gate.js)이 자동 검증합니다. 여기서 오류가 나오면 = 8대 원칙을 놓친 것이다.**
