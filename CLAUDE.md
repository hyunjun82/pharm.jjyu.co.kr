...
v1은 단일 공식 → 패턴 시그널. v2는 **5패턴 회전 + 가격 토큰 필수**.

**검색자가 입력하지 않는 토큰은 금지.** 연도(`2026`, `2025`), 괄호 부가 설명, 카테고리명 반복, AI 후킹(`총정리`) 모두 제거. **타이틀 ≤ 50자, 핵심 검색어만.**

### 5패턴 (모두 "가격" 토큰 필수)

```
① {제품명} 최저가 가격 | 성분 효과 복용법 부작용까지
② {제품명} 성분 효과 복용법 | 가격 부작용 주의사항
③ {제품명} 복용법 부작용 | 성분 효과 가격 비교
④ {제품명} 가격 성분 효과 | 복용법 부작용 주의사항
⑤ {제품명} 부작용 주의사항 | 성분 효과 복용법 가격
```

### 분산 규칙 (doorway 박멸 핵심)

- 같은 카테고리 안에서 **하나의 패턴이 30%를 초과하지 않음**
- 21개 글 → 패턴 ①~⑤ 각 4~5개씩 배분
- `verify-doorway.js`가 카테고리 내 타이틀 시작 토큰 분포 자동 검사

### 패턴 vs 검색의도 매핑 (참고)

| 패턴 | 어울리는 의도 | 본문 비중 메인 |
|---|---|---|
| ① 최저가 가격 | price | 가격 비교 40% |
| ② 성분 효과 복용법 | 기본·usage_scenario | 성분·복용법 40% |
| ③ 복용법 부작용 | safety·side_effect_avoid | 부작용·주의 40% |
| ④ 가격 성분 효과 | price + 성분 강조 | 가격 30% + 성분 30% |
| ⑤ 부작용 주의사항 | safety 강화 | 부작용·금기 50% |
